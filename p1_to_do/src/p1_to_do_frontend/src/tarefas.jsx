...
function tarefas() {
  ...
  async function consultarTarefas() {
    const lista = await p1_to_do_backend.getTarefas();
    setTarefas(lista);
  }

  // Obtém os totalizadores de tarefas do backend
  async function obterTotais() {
    const totalAndamento = parseInt(await p1_to_do_backend.totalTarefasEmAndamento());
    const totalConcl = parseInt(await p1_to_do_backend.totalTarefasConcluidas());
  ...
  async function excluir(id) {
    await p1_to_do_backend.excluirTarefa(parseInt(id));
    atualizarDados();
  }
